% FRED-MD DATA PREPROCESSING PIPELINE
% Adapted by: Tommaso Zipoli (2026)
% Source: Michael W. McCracken and Serena Ng (2017)
% -------------------------------------------------------------------------
...
% Add the folder containing the auxiliary function to the path
addpath(script_dir);
...
% Define data path and return to original directory
data_path = fullfile(root_dir, 'data', 'raw', filename);
cd(current_pwd);

% Load data from CSV file
if exist(data_path, 'file')
    dum=importdata(data_path,',');
else
...
% Export to CSV
df = array2table(yt, 'VariableNames', series);
df = addvars(df, dates, 'Before', 1);
processed_path = fullfile(root_dir, 'data', 'processed', filename);
writetable(df, processed_path);
fprintf('Exported: %s\n', processed_path);
